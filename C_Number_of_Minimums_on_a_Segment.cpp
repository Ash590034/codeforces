...
pl query(int start,int end,int index,int l,int r){ //O(logN) //l and r are arr Indexes and start and end are segtree node-ranges 
    if(l<=start && end<=r) return segTree[index];
    if(l>end || r<start) return {LLONG_MAX,end-start+1};
    int mid=(start+end)/2;
    pl lv=query(start,mid,2*index,l,r);
    pl rv=query(mid+1,end,2*index+1,l,r);
    return merge(lv,rv);
} 
...
void solve()
{
    ...
    cin>>n>>m;
    segTree.resize(4*n);
    for(int i=0;i<4*n;i++) segTree[i]={LLONG_MAX,1};
    vl arr(n);
    cin>>arr;
    build(arr,0,n-1,1);
    for(int i=0;i<m;i++){
        ...
        if(op==1){
            ll ind,val;
            cin>>ind>>val;
            update(arr,0,n-1,1,ind,val);
        }
        else{
            ll l,r;
            cin>>l>>r;
            r--;
            pl ans=query(0,n-1,1,l,r);
...
// pl query(int start, int end, int index, int l, int r) { // O(logN)
//     if (l <= start && end <= r) return segTree[index];
//     if (l > end || r < start) return {LLONG_MAX, 0};
//     int mid = (start + end) / 2;
//     pl lv = query(start, mid, 2 * index, l, r);
//     pl rv = query(mid + 1, end, 2 * index + 1, l, r);
//     return merge(lv, rv);
...
//     ll n, m;
//     cin >> n >> m;
//     segTree.resize(4 * n, {LLONG_MAX, 0});
//     vector<ll> arr(n);
//     for (ll &x : arr) cin >> x;
//     build(arr, 0, n - 1, 1);
    
//     // Debug: Print initial segment tree
//     //cout << "Segment Tree after build:" << endl;
//     // for (int i = 1; i < 4 * n; i++) {
//     //     cout << "Index " << i << ": (" << segTree[i].first << ", " << segTree[i].second << ")" << endl;
//     // }
...
//             // Debug: Print segment tree after update
//             // cout << "Segment Tree after update (index " << ind << " to " << val << "):" << endl;
//             // for (int i = 1; i < 4 * n; i++) {
//             //     cout << "Index " << i << ": (" << segTree[i].first << ", " << segTree[i].second << ")" << endl;
//             // }
//         } else {
