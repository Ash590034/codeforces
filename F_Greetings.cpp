...
void solve()
{
    ...
    sort(all(arr));
    vl newArr;
    for(auto it:arr) newArr.pb(it.s);
    //cout<<newArr<<endl;
    ll ans=0;
    function<void(int,int)> mS=[&](int l,int r){
        ...
        while(j<=r) temp.pb(newArr[j++]);
        for(int i=l;i<=r;i++) newArr[i]=temp[i-l];
        return;
    };
    // function<void(int, int)> mS = [&](int l, int r) {
    //     if (l >= r) return;
    ...
    //     while (i <= mid && j <= r) {
    //         if (newArr[i] < newArr[j]) {
    //             ans += (j - (mid + 1)); // Count of elements on the right side which are smaller
    ...
    //     while (i <= mid) {
    //         ans += (r - mid); // All elements from mid+1 to r are smaller
    //         temp.push_back(newArr[i++]);
    //     }
