...
void solve()
{
    ll n,c;
    cin>>n>>c;
    vl stair(n-1) , elevator(n-1);
    cin>>stair>>elevator;
    vvl dp(n,vl(2,1e9)); //dp[i][j] represents min time to reach ith floor from 0th floor if j=0 by stair and j=1 by elevator
    dp[0][0]=0;
    dp[0][1]=c;
    for(int i=1;i<n;i++){
        dp[i][0]=min(dp[i][0],dp[i-1][0] + stair[i-1]);
        dp[i][0]=min(dp[i][0],dp[i-1][1] + stair[i-1]);
        dp[i][1]=min(dp[i][1],dp[i-1][0] + elevator[i-1] + c);
