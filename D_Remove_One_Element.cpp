...
void solve()
{
    ...
    vl arr(n);
    cin>>arr;
    vl dp(n,1);//dp[i] tells the longest inc subarr ending at i;
    for(int i=1;i<n;i++){
        if(arr[i]>arr[i-1]){
            dp[i]=1+dp[i-1];
        }
    }
    vl suff(n,1);// suff[i] tells the lonegest inc subarr start with ith el.
    for(int i=n-2;i>=0;i--){
    ...
    for(int i=0;i<n;i++){
        ...
        if(i>0 && i<n-1 && arr[i-1]<arr[i+1]) maxi=max(maxi,dp[i-1]+suff[i+1]); 
    }
    cout<<maxi<<endl;
