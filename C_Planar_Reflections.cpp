...
ll n,k;
ll helper(ll i,ll k,ll d,vector<vvl> &dp){
    if(k==0) return 0;
    if(i==-1 || i==n) return 1;
    if(dp[i][k][d]!=-1) return dp[i][k][d];
    ll org=0,ref=0;
    if(d==0){
        org=helper(i-1,k,0,dp);
        ref=helper(i+1,k-1,1,dp);
    }
    else{
        ref=helper(i-1,k-1,0,dp);
        org=helper(i+1,k,1,dp);
    }
    return dp[i][k][d]=(org + ref)%M;
}
void solve()
{
    cin>>n>>k;
    vector<vvl> dp(n,vvl(k+1,vl(2,-1)));
    cout<<helper(0,k,1,dp)<<endl;
