...
void solve()
{
    ...
    if(sum&1) cout<<0<<endl;
        ...
        //vvl dp(n,vl(sum/2+1,-1));
        vl dp(sum/2+1,0);
        dp[0]=1;
        for(int i=1;i<=n;i++){
            for(int j=sum/2;j>=1;j--){
                if(i<=j) dp[j]=(dp[j-i] + dp[j])%mod;
            }
        }
        cout<<dp[sum/2]/2<<endl;
        //cout<<(modInv(2)*(89938794))%mod<<endl;
        ...
        //     ll skip=f(i+1,j);
        //     ll pick=0;
        //     if(i+1<=j) pick=f(i+1,j-i-1);
        //     return dp[i][j]=(pick + skip)%mod;
        // };
        // cout<<f(0,sum/2)/2<<endl;
    }
}
int32_t main()
{
