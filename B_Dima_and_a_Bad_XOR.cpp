...
void solve()
{
    ...
    // dp[i][0] tells if its possible to select even ones from 0 to i , 
    // dp[i][1] tells if its poss to select odd ones from 0 to i
    // for(int k=0;k<10;k++){
    //     vvl dp(n,vl(2,0));
    //     for(int j=0;j<m;j++) {
    //         if((arr[0][j]>>k)&1) dp[0][1]=1;
    //         else dp[0][0]=1;
    //     }
    //     for(int i=1;i<n;i++){
    //         for(int j=0;j<m;j++){
