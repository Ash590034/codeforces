...
//const int M=1e9 +7;
long long binExpMod(long long base, long long exp, long long mod) {
    long long result = 1;
...
ll calc(ll l, ll r,ll k){
    if(r==0) return 1LL;
    if(k==1) {
        //cout<<(binExpMod(10LL,r,M))%M<<endl;
        return (binExpMod(10LL,r,M))%M;
    }
    if(k==2){
        ll ans=binExpMod(5,r,M);
        //cout<<(ans)%M<<endl;
        return (ans)%M;
    }
    if(k==3){
        ll ans=binExpMod(4,r,M);
        //cout<<(ans)%M<<endl;
        return (ans)%M;
    }
    if(k==4){
        ll ans=binExpMod(3,r,M);
        //cout<<(ans)%M<<endl;
        return (ans)%M;
    }
    else{
        ll ans=binExpMod(2,r,M);
        //cout<<(ans)%M<<endl;
        return (ans)%M;
    }
}
void solve()
{
    ...
    cout<<(calc(0,r,k) - calc(0,l,k) + M )%M<<endl;
}
