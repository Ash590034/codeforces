...
ll help(ll l, ll r,vl &arr, vvl &dp){
    if(l>=r) return 0;
    if(dp[l][r]!=-1) return dp[l][r];

    if(arr[l]==arr[r]) return dp[l][r]=1 + help(l+1,r-1,arr,dp);
    else return dp[l][r]=1 + min(help(l+1,r,arr,dp),help(l,r-1,arr,dp));
}
void solve()
{
    ll n;
    ...
    for(int i=1;i<n;i++){
        ll x;
        cin>>x;
        if(x!=arr.back()) arr.pb(x);
    }
    n=arr.size();
    vvl dp(n,vl(n,-1));
    cout<<help(0,n-1,arr,dp)<<endl;
}
