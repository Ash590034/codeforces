...
void solve()
{
    ...
    for(int i=0;i<n;i++){
        ...
        cin>>sNo>>k>>s;
        if(sNo==1){
            for(auto it:s) hash1[it-'a']+=k;
        }
        else for(auto it:s) hash2[it-'a']+=k;
        if(hash2.size()>1) py;
        else if(hash2.size()==1){
            if(hash1.size()==1 && hash1[0]<hash2[0]) py;
            else if(hash1.size()>1 && hash1[0]>hash2[0]) pn;
            else pn;
        }
    }
