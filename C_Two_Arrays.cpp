...
const int MOD = 1e9 + 7;  // You can change the modulus value as needed
...
void precomputeFactorialsAndInverses(vector<long long>& fact, vector<long long>& invFact, int n, int mod) {
    fact[0] = 1;
    for (int i = 1; i <= n; ++i) {
        fact[i] = fact[i - 1] * i % mod;
    }
    invFact[n] = modExp(fact[n], mod - 2, mod); // Fermat's little theorem for modular inverse
    for (int i = n - 1; i >= 0; --i) {
        invFact[i] = invFact[i + 1] * (i + 1) % mod;
    }
}

// Function to compute nCr % p using precomputed factorials and inverses
...
void solve()
{
    ll n,m;
    cin>>n>>m;
    // Change this to the maximum value of n needed
    vector<long long> fact(3000), invFact(3000);

    // Precompute factorials and inverses up to n
    precomputeFactorialsAndInverses(fact, invFact, 2999 , MOD);
    cout<<nCr(n+2*m-1,2*m,fact,invFact,M)<<endl;
}
