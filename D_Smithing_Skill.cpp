...
void solve()
{
    ...
    vpl data;
    for(int i=0;i<n;i++){
        if(data.size()==0 || data.back().s>(arr[i].f-arr[i].s)){
            data.pb({arr[i].f,arr[i].f-arr[i].s});
        }
    }
    ll ans=0;
    ll ptr=0;
    vl dp(2e6+5,0);
    for(int i=1;i<=2e6;i++){
        while(ptr<data.size() && data[ptr].f<=i) ptr++;
        if(ptr) dp[i]=2 + dp[i-data[ptr-1].s];
    }
    for(auto &it:stock){
        if(it>data.back().f){
            ll diff=it-data.back().f;
            ans+=2*(diff/data.back().s);
            it-=diff/data.back().s*(data.back().s);
        }
        ans+=dp[it];
    }
    cout<<ans<<endl;
