...
ll dp[10][1<<10];
ll n,m;
ll help(int i,int mask,vector<string> &arr){
    if(i==arr.size()){
        if(__builtin_popcount(mask)==m) return 0;
        return 20;
    }
    if(dp[i][mask]!=-1) return dp[i][mask];
    ll mn=help(i+1,mask,arr);
    vl idx;
    for(int j=0;j<m;j++){
        if(arr[i][j]=='o' && (mask&(1<<j))==0) idx.pb(j);
    }
    ll newMask=mask;
    for(auto it:idx) newMask+=(1<<it);
    mn=min(mn,1 + help(i+1,newMask,arr));
    return dp[i][mask]=mn;
}
void solve()
{
