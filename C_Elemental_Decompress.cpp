...
void solve()
{
    ...
    for(int i=n-1;i>=0;i--){
        if(pp.count(arr[i].f)==0){
            //cout<<"Insert in p"<<endl;
            pp.insert(arr[i].f);
            p[arr[i].s]=arr[i].f;
        }
        else{
            //cout<<"Insert in q"<<endl;
            qq.insert(arr[i].f);
            q[arr[i].s]=arr[i].f;
        }
    }
    ll pptr=n , qptr=n;
    for(int i=n-1;i>=0;i--){
        if(pp.count(arr[i].f)==1 && q[arr[i].s]==0){
            while(qq.count(qptr)==1) qptr--;
            qq.insert(qptr);
            q[arr[i].s]=qptr;
            qptr--;
        }
        else{
            while(pp.count(pptr)==1) pptr--;
            pp.insert(pptr);
            p[arr[i].s]=pptr;
            pptr--;
        }
