...
void solve()
{
    ...
    vl arr(n);
    cin>>arr;
    int onePos , twoPos , mxPos;
    for(int i=0;i<n;i++){
        if(arr[i]==1) onePos=i+1;
        if(arr[i]==2) twoPos=i+1;
        if(arr[i]==n) mxPos=i+1;
    }
    ll mn=min(onePos,twoPos);
    ll mx=max(onePos,twoPos);
    if(mn<mxPos && mxPos<mx) cout<<mn<<" "<<mn<<endl;
    else{
        if(mxPos>mx) cout<<mx<<" "<<mxPos<<endl;
        else cout<<mn<<" "<<mxPos<<endl;
    }
}
