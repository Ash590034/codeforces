...
unsigned long long nCrModP(int n, int r, int p) {
    ...
    unsigned long long result = facts[n];
    result = (result * modInverse(facts[r], p)) % p;
    result = (result * modInverse(facts[n - r], p)) % p;
...
int32_t main()
{
    ...
    #endif
    for(ll i=1;i<=1000;i++) facts[i]=(facts[i-1]*i)%M;
    int t;
    cin>>t;
    while(t--)
    {
