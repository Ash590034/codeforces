...
void solve()
{
    ...
    vl newArr;
    for(int i=0;i<n;i++){
        if(newArr.size()==0 || newArr.back()!=arr[i]) newArr.pb(arr[i]);
    }
    for(int i=0;i<newArr.size();i++){
        if(i==0) prof[newArr[i]]++;
        else if(i+1==newArr.size()) prof[newArr[i]]++;
        else if(newArr[i-1]==newArr[i+1]) prof[newArr[i]]+=2;
        else prof[newArr[i]]++;
    }
    ll mx=1;
    for(int i=2;i<=k;i++){
        if(prof[i]>prof[mx]) mx=i;
    }
    cout<<mx<<endl;
}
