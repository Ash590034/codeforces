...
void solve()
{
    ...
    vl arr(n);
    cin>>arr;
    pl dp[1<<n]; //dp[subset] tells the min trips and current wt for current subset 
    dp[0]={1,0};
    for(int sub=1;sub<(1<<n);sub++){
        dp[sub]={INT_MAX,INT_MAX};
        for(int i=0;i<n;i++){
            if(((1<<i)&sub)){
                pl current=dp[sub^(1<<i)];
                if(current.second + arr[i] <=x) current.second+=arr[i];
                else {
                    current.first++;
                    current.second=arr[i];
                }
                dp[sub]=min(dp[sub],current);
            }
        }
    }
