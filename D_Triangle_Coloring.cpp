...
// Function to compute modular inverse of a % p using Fermat's Little Theorem
...
unsigned long long nCrModP(int n, int r, int p) {
    ...
    // Compute nCr % p using the factorials and modular inverses
    unsigned long long result = fact[n];
    result = (result * modInverse(fact[r], p)) % p;
    result = (result * modInverse(fact[n - r], p)) % p;
...
void solve()
{
    ...
    for(int i=1;i<n/3+1;i++) fact[i]=(fact[i-1]*i)%mod;
    ll ans=nCrModP(n/3,n/6,mod);
    for(int i=0;i<n;i+=3){
        vl curr(3);
        for(int j=i;j<i+3;j++) curr[j-i]=arr[j];
        sort(all(curr));
        if(curr[0]==curr[2]) ans=(ans*3)%mod;
        else if(curr[0]==curr[1]) ans=(ans*2)%mod;
    }
    cout<<ans<<endl;
}
