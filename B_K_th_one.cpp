...
void update(vector<ll> &arr,int start,int end,int index,int pos){ //O(logN)
    if(start==end){
        arr[pos]^=1;
    ...
    if(mid>=pos){
        update(arr,start,mid,left,pos);
    }
    else{
        update(arr,mid+1,end,right,pos);
    }
    segTree[index]=merge(segTree[left] , segTree[right]);
}
ll query(int start,int end,int index,int k){ //O(logN) //l and r are arr Indexes and start and end are segtree node-ranges 
    if(start==end){
        return start;
    }
    ll left=2*index ,right=2*index+1;
    ll mid=(start+end)/2;
    if(segTree[left]>=k) return query(start,mid,left,k);
    else return query(mid+1,end,right,k-segTree[left]);    
} 
