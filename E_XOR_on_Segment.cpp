...
struct LazySGT {
    ...
    void pushdown(int index, int start, int end) {
        if (lazy[index]) {
            ...
            lazy[index] = false; // Reset lazy state after pushdown
        }
    }
...
struct Update1 {
    ...
    void apply(Node1 &a, int start, int end) {
        if (val == 0) return;
        for (int i = 0; i < 32; i++) {
            if (val & (1 << i)) a.bits[i] = (end - start + 1) - a.bits[i];
        }
    }
...
void solve() {
    ...
    for (int i = 0; i < m; i++) {
        ...
        if (op == 1) {
            ...
            l--;
            r--;
            vl bits = tree.make_query(l, r).bits;
            ll ans = 0;
            for (int i = 0; i < 32; i++) ans += (bits[i] * (1LL << i));
            cout << ans << endl;
        } else {
            ll l, r, v;
            cin >> l >> r >> v;
...
// struct Node1 {
//     vector<ll> bits;// may change
//     Node1() { // Identity element
//         bits.assign(32,0);    // may change
//     }
...
//         if(val==0) return;
//         for(int i=0;i<32;i++){
//             if(val&(1<<i)) a.bits[i]=(start-end+1)-a.bits[i];
//         }
//     }
//     void combine(Update1& new_update, int start, int end){
//         val ^= new_update.val;
...
//             vl bits=tree.make_query(l,r).bits;
//             ll ans=0;
//             for(int i=0;i<32;i++) ans+=(bits[i]*1LL*(1<<i));
//             cout<<ans<<endl;
//         }
//         else{
//             ll l,r,v;
