...
long long bme(long long base, long long exponent, long long modulus) {
    // Initialize result to 1 (base^0 % modulus is 1)
    long long result = 1;
    
    // Ensure base is within the modulus to avoid overflow
    base = base % modulus;

    // Main loop to perform binary exponentiation
    while (exponent > 0) {
...
void solve()
{
    ...
    for(int i=0;i<q;i++){
        ...
        ll oneContri=(bme(2,ones,M) - 1 + M)%M;
        ll zeroContri=(bme(2,len-ones,M) - 1 + M)%M;
        zeroContri=(zeroContri*oneContri)%M;
        cout<<(oneContri + zeroContri)%M<<endl;
    }    
}
