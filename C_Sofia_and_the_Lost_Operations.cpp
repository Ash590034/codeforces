...
void solve()
{
    ...
    map<ll,vl> mppMod;
    for(int i=0;i<m;i++){
        mppMod[modVal[i]].pb(i);
    }
    multiset<ll> st(all(fin));
    for(int i=0;i<n;i++){
        if(org[i]!=fin[i]){
            if((mppMod.find(fin[i])==mppMod.end()) || (mppMod[fin[i]].size()==0)){
                pn;
                //cout<<"bef"<<endl;
                return;
            }
            else{
                modVal[mppMod[fin[i]].back()]=0;
                mppMod[fin[i]].pop_back();
            }
        }
    }
    if(modVal.back()==0) py;
    else if(st.find(modVal.back())!=st.end()) py;
    else {
        pn;
        //cout<<"aft"<<endl;
    }
