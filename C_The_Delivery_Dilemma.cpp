...
bool check(vl &a,vl &b,ll mid){
    ll maxi=0;
    ll sum=0;
    for(int i=0;i<a.size();i++){
        if(a[i]<=mid) maxi=max(maxi,a[i]);
        else{
            sum+=b[i];
        }
    }
...
void solve()
{
    ...
    for(int i=n-2;i>=0;i--) suff[i]+=suff[i+1]+arr[i].second;
    ll mini=suff[0];
    for(int i=0;i<n;i++){
        ll curr=max(arr[i].first,(i+1<n ? suff[i+1]:0LL));
        mini=min(mini,curr);
    }
    cout<<mini<<endl;
    ...
    // ll low=0;
    // ll high=min(sum,maxi);
    // while(low<=high){
    //     ll mid=(low+high)>>1;
    //     if(check(a,b,mid)) high=mid-1;
    //     else low=mid+1;
    // }
    // cout<<low<<endl;
