...
ll n;
ll help(int i,int mask,vvl &pers,vvl &dp){
    if(i==101) return (__builtin_popcount(mask)==n);
    if(dp[i][mask]!=-1) return dp[i][mask];
    
    ll ways=help(i+1,mask,pers,dp); //SKIP
    for(auto per:pers[i]){
        if(!(mask&(1<<per))) ways=(ways + help(i+1,mask + (1<<per),pers,dp))%M;
    }
...
void solve()
{
    ...
    for(int i=0;i<n;i++){
        ...
        string shirt;
        while(s>>shirt){
            pers[stoi(shirt)].pb(i);
        }
    }
    cout<<help(1,0,pers,dp)<<endl;
    return;
}
