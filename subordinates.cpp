...
//     return ans;
// }
// void dfs(int node ,int par,vvl &adj,vl &ans,ll &time){
//     ll inTime=time;
//     for(auto it:adj[node]){
//         if(it==par) continue;
//         time++;
//         dfs(it,node,adj,ans,time);
//     }
//     ans[node]=time-inTime;
// }
// void solve()
// {
//     ll n;
...
void dfs(int node, int parent, const vector<vector<int>>& tree, vector<int>& subtreeSize) {
    subtreeSize[node] = 1; // Start with counting the node itself
    for (int child : tree[node]) {
        if (child != parent) { // Avoid revisiting the parent node
            dfs(child, node, tree, subtreeSize);
...
int main() {
    ...
    for (int i = 1; i <= n; i++) {
        cout << subtreeSize[i] - 1 << " "; // Subordinates count
    }
    cout << endl;
