...
void solve()
{
    ...
    for(int i=0;i<p+t+1;i++){
        if(pro.size()==1+p){
            if(pro.find(i)!=pro.end()) cout<<ans-a[i]<<" ";
            else cout<<ans-b[i]-a[*pro.rbegin()] + b[*pro.rbegin()]<<" ";
        }
        else if(tes.size()==1+t){
            if(tes.find(i)!=tes.end()) cout<<ans-b[i]<<" ";
            else cout<<ans-a[i]-b[*tes.rbegin()] + a[*tes.rbegin()]<<" ";
        }
    }
    cout<<endl;
