...
void solve()
{
    ...
    for(int i=1;i<=m;i++) prefOps[i]+=prefOps[i-1];
    vl opArr(n+1,0);
    for(int i=0;i<m;i++){
        ll noOfTimes=prefOps[i];
        ll l=ops[i][0];
        ll r=ops[i][1];
        ll d=ops[i][2];
        l--;
        r--;
        opArr[l]+=d*noOfTimes;
        opArr[r+1]-=d*noOfTimes;
    }
    for(int i=1;i<n;i++) opArr[i]+=opArr[i-1];
    for(int i=0;i<n;i++) cout<<arr[i] + opArr[i]<<" ";
    cout<<"\n";
