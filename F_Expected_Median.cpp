...
const int MOD = 1e9 + 7;
const int MAXN = 300000;  // Set an appropriate limit based on your needs
...
// Function to compute factorials and inverse factorials % MOD
...
void solve()
{
    ...
    ll zero=n-tot;
    ll ans=0;
    for(ll i=(k+1)/2;i<=min(tot,k);i++){
        ll curr=(nCm(tot,i)*nCm(zero,k-i))%M;
        ans=(ans + curr)%M;
    }
    cout<<ans<<endl;
}
