...
ll check(vector<long double> &arr,long double mid){
    ll cnt=0;
    for(auto l:arr) cnt+=(ll)(l/mid);
    return cnt;
}
void solve()
{
    ...
    cin>>arr;
    long double low=0 , high=accumulate(all(arr),0.0)/(k*1.0);
    long double ans;
    for(int i=0;i<80;i++){
    ...
    cout<<fixed<<setprecision(12)<<ans; //ANSWER ALWAYS IN THIS WAY
}
