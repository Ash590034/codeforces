...
ll power(ll base, ll exp, ll mod) {
    ...
    while (exp > 0) {
        ...
        // Now exp must be even, so halve it
        exp = exp >> 1;
        // Square the base
        base = (base * base) % mod;
    }
...
vl maxArr(26);
vl facts(1001,1);
ll help(ll i,vl &curr,vector<map<vl,ll>> &dp){
    if(i==k){
        ll ans=facts[k];
    ...
    ll ways=0;
    for(int j=0;j<26;j++){
        if(curr[j]+1<=maxArr[j]){
            curr[j]++;
            ways=(help(i+1,curr,dp) + ways)%mod;
            curr[j]--;
        } 
    }
...
void solve()
{
    for(ll i=1;i<=1000;i++) facts[i]=(facts[i-1]*i)%mod;
    cin>>k;
    cin>>maxArr;
    vector<map<vl,ll>> dp(k+1);
    vl curr(26,0);
    cout<<help(0,curr,dp)<<endl;
}
