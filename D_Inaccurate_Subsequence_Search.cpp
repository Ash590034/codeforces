...
void solve()
{
    ...
    for(int i=0;i<m;i++){
        ...
        else buffer.insert(a[i]);
    }
    if((int)sta.size()>=k) ans++;
    for(int i=m;i<n;i++){
        if(sta.find(a[i-m])!=sta.end()){
            if(buffer.find(a[i-m])!=buffer.end()){
                buffer.erase(buffer.find(a[i-m]));          
            }
            else{
                sta.erase(sta.find(a[i-m]));
                stb.insert(a[i-m]);
            }
        }
        if(stb.find(a[i])!=stb.end()){
