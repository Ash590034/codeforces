...
//     segTree[index]=merge(segTree[left] , segTree[right]);
// }
// ll query(int start,int end,int index,int l,int x,vl &arr){ //O(logN) //l and r are arr Indexes and start and end are segtree node-ranges 
//     if(start==end) return start;
//     ll mid=(start+end)/2;
//     ll left=2*index , right=2*index+1;
//     if(mid>=l){
//         ll lv=query(start,mid,left,l,x,arr);
//         ll rv=query(mid+1,end,left,l,x,arr);
//         if(arr[lv]>=x) return lv;
//         return rv;
//     }
//     else{
//         return query(mid+1,end,right,l,x,arr);
//     }
// }
// void solve()
// {
...
//             ll j,x;
//             cin>>x>>j;
//             ll val=query(0,n-1,1,j,x,arr);
//             cout<<(arr[val]>=x ? val:-1)<<endl;
//         }
//     } 
// }
...
void build(vi &arr, int start, int end, int index) { // O(n)
    if (start == end) {
        segTree[index] = arr[start];
        return;
    }
...
void update(vi &arr, int start, int end, int index, int pos, int val) { // O(logN)
    if (start == end) {
        arr[pos] = val;
        segTree[index] = arr[pos];
        return;
...
int query(int start, int end, int index, int l, int x, vi &arr) { // O(logN)
    ...
    if (mid >= l) {
        int res = query(start, mid, left, l, x, arr);
        if (res != -1) {
            return res;
        }
    }
...
void solve() {
    ...
    for (int i = 0; i < m; ++i) {
        ...
        if (op == 1) {
            ...
            int x, j;
            cin >> x >> j;
            int val = query(0, n - 1, 1, j, x, arr);
            cout << ((val != -1 && arr[val] >= x) ? val : -1) << endl;
        }
    }
}
