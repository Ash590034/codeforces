...
void solve()
{
    ...
    for(int i=0;i<n;i++){
        minDiff=min(minDiff,llabs(a[i]-b[n]));
        if((a[i]-b[n]>0 && b[i]-b[n]<0) || (a[i]-b[n]<0 && b[i]-b[n]>0)) minDiff=0;
        ans+=llabs(a[i]-b[i]);
        minDiff=min(minDiff,llabs(b[i]-b[n]));
    }
    if(minDiff==LLONG_MAX) cout<<ans<<endl;
    else cout<<ans+minDiff<<endl;
