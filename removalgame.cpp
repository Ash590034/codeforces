...
void solve()
{
    ...
    vl arr(n);
    cin>>arr;
    vvl dp(n,vl(n,1e15)); //dp[i][j] max possible score for arr start at i and end at j
    vl pref(n);
    pref[0]=arr[0];
    ...
    for(int i=n-1;i>=0;i--){
        for(int j=0;j<n;j++){
            ...
            if(i==j){
                ...
                dp[i][j]=max(arr[i] + pref[j] - pref[i] - dp[i+1][j] ,arr[j] + pref[j-1] -(i>0 ? pref[i-1]:0) - dp[i][j-1] );
            }
        }
    }
    cout<<dp[0][n-1]<<endl;
