...
void solve()
{
    ...
    while(curr<copy){
       digs++;
       ll add=9;
       for(int i=1;i<(digs+1)/2;i++){
          add*=10;
       }
       curr+=add;
    }
    for(int j=1;j<=digs-1;j++){
        if(j==1) n-=10;
        else{
            n-=9*pow(10,(j-2+1)/2);
        }
    ...
    while(n>1){
        if(i==0){
            for(int d=1;d<=9;d++){
                ll more=pow(10,(digs - 2 + 1 - i*2)/2);
                if(more*d>n){
                    n-=more*(d-1);
                    ans[i]=ans[digs-1-i]=d;
                    break;
                }
            }
        }
