...
ll sumAP(ll lst,ll terms,ll diff){
    return terms*(lst + lst - (terms-1)*diff)/2;
}
void solve()
{
    ll n,k;
    ...
    ll cnt=0;
    for(int i=0;i<n;i++){
        if(a[i]<=lstVal) continue;
        ll currCnt=1 + (a[i] - lstVal - 1)/b[i];
        cnt+=currCnt;
        ans+=sumAP(a[i] , currCnt , b[i]);
    }
    ans+=(k-cnt)*lstVal;
    cout<<ans<<endl;  
}
